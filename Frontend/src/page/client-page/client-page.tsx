...
import { formatEther, parseEther } from "../../../node_modules/viem/_types";
...
const ClientPage: React.FC = () => {
  ...
  // App state
  const [activeTab, setActiveTab] = useState<"organizations" | "jobs" | "work">(
    "organizations"
  );
    ...
    if (selectedJob) {
      ...
      for (let i = 0; i < selectedJob.milestoneCount; i++) {
        milestones.push({
          ...
          amount: (
            parseFloat(selectedJob.totalPayment) / selectedJob.milestoneCount
          ).toFixed(3),
          deadline: 0,
          status:
            i === selectedJob.currentMilestone
              ? 1
              : i < selectedJob.currentMilestone
              ? 2
              : 0,
  ...
  const handleDepositFunds = () => {
    ...
    const updatedJobs = jobs.map((job) =>
      job.address === selectedJob.address ? { ...job, status: 1 } : job
    );
                  ...
                  {selectedOrgId && jobs.length > 0 ? (
                    ...
                    <div className="col-span-full text-center py-16 text-gray-500">
                      {selectedOrgId
                        ...
                        : "Select an organization to view its jobs"}
                    </div>
                  )}
                </div>
              </div>
              ...
              <div>
                <div className="mb-6">
                  ...
                  <p className="text-gray-600">
                    Jobs assigned to you as a worker
                  </p>
                </div>
                      ...
                      >
                        ...
                        <div className="mt-4 pt-4 border-t border-gray-100">
                          <div className="flex justify-between items-center">
                            <div className="text-sm text-gray-500">
                              Employer:{" "}
                              {`${job.employer.substring(
                                0,
                                6
                              )}...${job.employer.substring(
                                job.employer.length - 4
                              )}`}
                            </div>
                            <button className="text-indigo-600 hover:text-indigo-800">
                              Details
      ...
      {showCreateOrgModal && (
        <div className="fixed inset-0 bg-gray-900 bg-opacity-50 flex justify-center items-center z-50">
          <div className="bg-white rounded-lg p-8 max-w-md w-full">
            ...
            >
              <div className="mb-4">
                <label className="block text-gray-700 mb-2" htmlFor="org-name">
                  Organization Name
                </label>
                <input
                  type="text"
            ...
            >
              ...
              <div className="mb-4">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setNewJobMilestoneCount(parseInt(e.target.value))
                  }
                  min="1"
                  max="10"
                  required
          ...
          <div className="bg-white rounded-lg p-8 max-w-2xl w-full">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-bold">
                Set Milestones for {selectedJob?.title}
              </h3>
              <button
            ...
            <div className="mb-6">
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                ...
                <div>
                  ...
                  <p className="font-medium">
                    {selectedJob.currentMilestone}/{selectedJob.milestoneCount}{" "}
                    completed
                  </p>
                </div>
                <div>
                  ...
                  <p className="font-medium">
                    {selectedJob.employer.substring(0, 6)}...
                    {selectedJob.employer.substring(
                      selectedJob.employer.length - 4
                    )}
                  </p>
                </div>
                {selectedJob.worker && (
                  <div>
                    ...
                    <p className="font-medium">
                      {selectedJob.worker.substring(0, 6)}...
                      {selectedJob.worker.substring(
                        selectedJob.worker.length - 4
                      )}
                    </p>
                  </div>
                )}
              ...
              <div className="space-y-3">
                {jobMilestones.map((milestone, index) => (
                  <div
                    key={index}
                    className={`border rounded-lg p-4 ${getMilestoneStatusColor(
                      milestone.status
                    )}`}
                  >
                    <div className="flex justify-between items-center mb-2">
                      ...
                      <span
                        className={`px-2 py-1 rounded-full text-xs ${getMilestoneStatusColor(
                          milestone.status
                        )}`}
                      >
